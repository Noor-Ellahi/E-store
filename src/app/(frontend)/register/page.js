    ...
    const registerUser = async () => {
        try {
            const res = await axios.post(
                '/api/auth/register',
                { name: nameRef.current.value, email: emailRef.current.value, password: passRef.current.value, }
            )
        ...
        <div className="flex relative  justify-center items-center h-screen bg-[#FAFAFA]">
            ...
            <div className="flex flex-col gap-5 pt-10 pb-15 px-5 rounded-xl items-start w-100 mx-10">
                ...
                <div className="w-full relative">
                    <input ref={nameRef} type="text" placeholder="Username" className="peer block bg-white py-2.5 pl-2 rounded-sm focus:border-blue-500 border-1 border-[#DADCE0] border-solid w-[100%] outline-none placeholder-transparent" />
                    <label
                        htmlFor="Name"
                ...
                <button className="w-full bg-[#2240f5] text-[#fff] py-2 rounded-sm transition hover:shadow-lg cursor-pointer" onClick={registerUser}>Register</button>
